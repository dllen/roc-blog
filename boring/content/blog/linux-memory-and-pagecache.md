...
# Linux 内存与 PageCache 深度技术手册

## —— 内核机制 · NUMA · cgroup · 云原生数据库调优 · 实战排障
...
# 5️⃣ cgroup 对 PageCache 的隔离与限制机制
...
*   触发 `memcg reclaim`。
*   内核尝试只回收该 cgroup 内的 PageCache 和匿名页。
...
# 7️⃣ THP（Transparent Huge Page）对 PageCache 的影响
...
数据库在**裸机**上的问题：
> 往往是 **PageCache 与匿名页的全局竞争**问题。

数据库在**容器（K8s）**中的问题：
> 是 **memcg 限制 + PageCache 堆积 + NUMA 不均 + 脏页写回策略** 的叠加问题。
