---
title: "从 intent.md 到闭环：AI 原生软件开发的六个阶段、一条制品链、几道审批门"
date: 2026-09-02
update_date: 2026-09-02
description: "解读 Anthropic《AI 原生 SDLC 手册》：从 intent.md 到闭环的六个阶段、一条制品链、几道审批门，重新定义受监管企业如何在 agent 时代落地 Claude Code。"
tldr: "AI 原生 SDLC 不是把流程取消掉，而是把旧的控制目标翻译成新的执行机制——制品链、门、hook、eval、闭环。"
taxonomies:
  tags: ["AI", "LLM", "Agent", "Claude Code", "SDLC", "软件工程"]
---
...
2026 年 8 月 21 日，Anthropic 在 claude.com 博客上发布了一篇题为 The AI-Native SDLC playbook 的长文。文章标注阅读时间 5 分钟，实际正文接近一万个英文单词，附带十几段配置和 prompt 示例。它更接近一份操作手册，把 Anthropic 在企业客户那里落地 Claude Code 时积累的做法，按软件开发生命周期的六个阶段整理成了一组可以逐个采用的 play。
...
维护阶段发现的问题直接写成新的 intent 文件，重新进入计划阶段，而不再是"报一个 bug 等人来处理"。

### 2. 每个阶段以提交一个制品结束

每个阶段向版本控制提交一个制品，下一个阶段从读取这个制品开始：
...
早期阶段用 Markdown，理由是产品负责人和 agent 都能读、都能改同一个文件；从构建阶段往后，制品就是代码和它的记录。作者认为这条提交链本身就是审计轨迹，能够回答谁提了什么要求、agent 做了什么、谁批准的。

### 3. 人的注意力集中在门上

一个制品被接受，就触发下一阶段。一开始每一步都由人手动 prompt，目标状态是每个被接受的制品自动触发下一道门，人只在门口审阅 agent 标记出来的问题。
...
实施上有几个细节。非工程师通过 claude.ai 或 Cowork 使用 Claude，不需要会 git，通过 GitHub connector 代为提交。单个产品最简单的做法是在产品仓库里放一个 `intent/` 目录，让意图文件和后面派生出来的代码挨在一起；单独的 intent 仓库只在意图跨多个仓库时才划得来。模板可以写成 skill，由技术人员搭建、负责人签字。产品负责人审阅并修正 agent 写的 intent.md，然后提交，接受或拒绝以 merge 或关闭 review 的形式留下记录。
...
作者描述的变化是：从看着 agent 逐条改、逐个动作审，转为长时间自主运行之后审阅制品。这是后面并行工作和闭环自治的基础。
...
| 双向链接 | 所有制品记录 ID，所有遗留记录写 commit SHA，承认存在两个事实来源 | 过渡期的最低标准 |
...
构建期 hook 要快，只作用于改过的文件，重型检查放到 commit 或 PR。作者特意说明，需要人审批的 hook 不该放在构建阶段，因为它会把人放回所有并行会话的关键路径上。
...
作者把 eval 叫作阶段门 QA 的 AI 原生对应物。eval 套件在 agent 的配置变化时运行，换模型、改 prompt、改 CLAUDE.md、改 skill、改 hook 都算。理由是这些配置在引导 agent，应该得到和代码一样的回归测试。
...
2. **门后写操作**：修 lint、更新生成的文档、响应 @claude 评论
...
- **人不在并行会话的关键路径上。** 构建期不设人工审批 hook，人的审批集中在发布门和制品评审。

---

## 五、我的判断

读完这份手册，我最想强调的不是它教了什么流程，而是它把"受监管企业怎么在 agent 时代保持问责"这个问题，从辩论变成了工程。下面三条是我认为最值得带走、也最容易被只读一遍就滑过去的判断。

### 三条最值得带走的判断

**第一，"建议性控制 vs 确定性控制"是全文最重要的一个区分。** Skill 让违规变少，hook 让违规几乎不可能。这个二分法回答了一个普遍困惑：为什么写了 CLAUDE.md，agent 还是不听话？因为 CLAUDE.md、prompt、skill 都只是建议层的——它们让正确行为成为大概率事件，却没有能力阻止错误发生。必须成立的事，背后一定要有确定层的 hook、权限、沙箱或 CI 兜底。记住这一条，很多"AI 治理"的配置困惑会瞬间变清楚。

**第二，人工评审是这套体系隐藏的速率上限。** 文章反复说"并行会话数以评审跟得上为限"，却没追问一个更尖锐的问题：当所有 PR 都附带 agent 评审、所有测试都绿了之后，人靠什么保持批判性而不走过场？这是组织行为问题，不是工程问题——而它恰恰决定了整套体系能跑多快。工具可以把审查成本降到很低，但把"人是否认真"这件事工程化，目前还没有答案。

**第三，intent.md 是整条链上最脆弱、也被着墨最少的一环。** 手册把大量篇幅给了构建、测试、部署的工程细节，但意图定义——那个最不被 AI 放大、又最容易出错的第一步——只有几段。原因不难理解：Anthropic 是工程组织，它擅长的是"意图确定之后"的事。但如果意图本身错了，后面所有自动化都只是在更高效地做错事。这也是为什么这套框架落到真实组织，往往卡在第一步而不是最后一步。

### 长处

这篇文章正面回答了受监管企业最关心的问题：agent 写了大部分代码之后，问责、审计、职责分离、变更管理怎么办。它的答案是把这些控制目标全部映射到 git 制品、hook、分支保护和托管设置上，而不是取消它们。对合规、安全、平台工程的读者来说，这比"提效"的说法更有说服力。
...
总的来说，这份手册适合已经在用 agentic 编码工具、正感到流程跟不上代码的中大型工程组织，尤其是受监管行业。它的贡献在于给出了一套把旧的控制目标翻译为新的执行机制的完整词汇：**制品链、门、hook、eval、闭环**。即使不用 Claude，这套词汇也可以借用。
